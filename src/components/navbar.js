...
const Navbar = () => {
  ...
  <div className="flex items-center justify-between w-full gap-x-2 px-1 sm:px-2 md:px-4 relative flex-nowrap overflow-hidden">
    {/* Left Logo */}
 <img
  src={img2}
  alt="SIE Logo"
  className="h-10 sm:h-14 md:h-20 w-auto max-w-[80px] sm:max-w-[120px] md:max-w-[240px] lg:max-w-[280px] object-contain flex-shrink-0"
/>


    {/* Center Text */}
    <div className="flex-1 min-w-0 px-2 text-center overflow-hidden">
      <h1 className="truncate text-sm xs:text-base sm:text-xl md:text-3xl lg:text-4xl font-bold text-gray-800 leading-tight">
    ...
    <img
      src={img1}
      alt="IIT Madras Logo"
      className="h-10 xs:h-12 sm:h-16 md:h-20 w-auto max-w-[80px] sm:max-w-[100px] object-contain flex-shrink-0"
    />
              ...
              {activitiesLinks.map(({ name, link, submenu }, idx) => (
                <li key={name} className={`border-b border-black ${idx === activitiesLinks.length - 1 ? "border-none" : ""} relative`} ref={name === "Innosphere" ? innosphereRef : undefined}>
                  {submenu ? (
                    ...
                    <a href={link} target="_blank" rel="noopener noreferrer" className="block px-4 py-2 hover:bg-amber-400">{name}</a>
                  )}
                </li>
              ))}
            </ul>
          ...
          {isStudentsOpen && (
            <ul className="absolute top-full mt-1 left-0 w-72 bg-white text-black font-medium shadow-lg border-t border-black z-[999]">
              <li className="relative border-b border-black">
                <a href="https://oieiitm.org/ms_entrepreneurship/home" target="_blank" rel="noopener noreferrer" className="hover:bg-amber-400">
                  <button onClick={toggleSupportDropdown} className="w-full text-left px-4 py-2 hover:bg-amber-400 flex justify-between items-center">
                    M.S(E) Program
                  </button>
                </a>
              </li>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 md:hidden">
          <div className="bg-gradient-to-b from-white/90 to-amber-50/80 backdrop-blur-xl w-full max-w-[16rem] h-full p-0 fixed top-0 right-0 shadow-2xl rounded-l-xl transition-transform duration-300 translate-x-0 overflow-y-auto flex flex-col border border-amber-100">
            ...
            <nav className="flex-1 flex flex-col gap-3 text-xl font-semibold text-gray-800 px-6 py-6">
              <NavLink to="/" onClick={closeMenu} className={({ isActive }) => `py-3 px-3 rounded-lg transition ${isActive ? 'text-amber-500 font-bold bg-amber-100' : 'hover:bg-amber-100 active:bg-amber-200'}`}>Home</NavLink>
              <NavLink to="/about_us" onClick={closeMenu} className={({ isActive }) => `py-3 px-3 rounded-lg transition ${isActive ? 'text-amber-500 font-bold bg-amber-100' : 'hover:bg-amber-100 active:bg-amber-200'}`}>About</NavLink>
              <NavLink to="/news" onClick={closeMenu} className={({ isActive }) => `py-3 px-3 rounded-lg transition ${isActive ? 'text-amber-500 font-bold bg-amber-100' : 'hover:bg-amber-100 active:bg-amber-200'}`}>News</NavLink>

              {/* Activities Dropdown */}
              <div>
                <button onClick={toggleMobileEventsDropdown} className="flex justify-between items-center w-full py-3 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition font-semibold">
                  Activities
                  <FaChevronDown className={`${isMobileEventsOpen ? "rotate-180" : ""} transition-transform`} />
                </button>
                {isMobileEventsOpen && (
                    ...
                    {activitiesLinks.map(({ name, link, submenu }) => (
                      submenu ? (
                        <div key={name} className="relative">
                          <button onClick={toggleMobileInnosphereDropdown} className={`flex justify-between items-center w-full text-lg py-2 px-3 rounded-lg ${
                            (submenu.some(sub => location.pathname === sub.link)) ? 'text-amber-500 font-bold bg-amber-100' : 'hover:text-amber-500'
                          }`}>
                            {name}
                            <FaChevronDown className={`ml-2 text-xs ${isMobileInnosphereOpen ? 'rotate-180' : ''}`} />
                          </button>
                          {isMobileInnosphereOpen && (
                            <div className="ml-4 mt-1 space-y-2">
                              {submenu.map((sub) => (
                                <a key={sub.name} href={sub.link} target="_blank" rel="noopener noreferrer" onClick={closeMenu} className="block text-lg py-2 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">{sub.name}</a>
                              ))}
                            </div>
                          )}
                        </div>
              ...
              {/* Academics */}
              <a href="https://oieiitm.org/ms_entrepreneurship/home" target="_blank" rel="noopener noreferrer" onClick={closeMenu} className="py-3 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">Academics</a>

              {/* Students Dropdown */}
              {/* <div>
                <button onClick={toggleMobileStudentsDropdown} className="flex justify-between items-center w-full py-3 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition font-semibold">
                  Students
                  <FaChevronDown className={`${isMobileStudentsOpen ? "rotate-180" : ""} transition-transform`} />
                </button>
                {isMobileStudentsOpen && (
                  <div className="ml-4 mt-1 space-y-2">
                    <button onClick={toggleMobileSupportDropdown} className="flex justify-between w-full text-lg py-2 px-3 rounded-lg hover:text-amber-500">
                      Support
                      <FaChevronDown className={`${isMobileSupportOpen ? "rotate-180" : ""} transition-transform`} />
                    </button>
                    {isMobileSupportOpen && (
                      <div className="ml-4 mt-1">
                        <a href="#" onClick={closeMenu} className="block text-lg py-2 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">UGFIR</a>
                        <a href="#" onClick={closeMenu} className="block text-lg py-2 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">PGFIR</a>
                      </div>
                    )}
                  </div>
                )}
              ...
              {/* <a href="#" onClick={closeMenu} className="py-3 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">Parents</a>
              <a href="#" onClick={closeMenu} className="py-3 px-3 rounded-lg hover:bg-amber-100 active:bg-amber-200 transition">Admissions</a> */}
              <NavLink to="/contact" onClick={closeMenu} className={({ isActive }) => `py-3 px-3 rounded-lg transition ${isActive ? 'text-amber-500 font-bold bg-amber-100' : 'hover:bg-amber-100 active:bg-amber-200'}`}>Contact</NavLink>
            </nav>
          </div>
        </div>
      )}
